...
namespace TripTales.Webapi.Controllers
{
    ...
    public class UserController : EntityReadController<User>
    {
        ...
        public async Task<IActionResult> CreateEmailToken()
        {
            ...
            await _emailSender.SendEmailAsync(user.Email, "TripTales Email Change", $"<p>Beim folgenden Link kann die Email ge�ndert werden: <a href='{_config["RedirectEmailChange"]}{token}'>{_config["RedirectEmailChange"]}{token}</a>.<br><p>Mit freundlichen Gr��en</p><br><p>Ihr TripTales Team</p>");
            return Ok();
        }
        ...
        public async Task<IActionResult> ForgotPassword(string email)
        {
            ...
            await _emailSender.SendEmailAsync(email, "TripTales Password Reset", $"<p>Beim folgenden Link kann das Password zur�ckgesetzt werden: <a href='{_config["RedirectPasswordReset"]}{token} '> {_config["RedirectPasswordReset"]}{token}</a>.<br><p>Mit freundlichen Gr��en</p><br><p>Ihr TripTales Team</p>");
        ...
        public async Task<IActionResult> GetUserByRegistryName(string registryName)
        {
            ...
            var user = await _db.User.Include(a => a.Posts).ThenInclude(a => a.Likes).Include(a => a.FollowerRecipient).ThenInclude(a => a.Sender).Include(a => a.FollowerSender).ThenInclude(a => a.Recipient).FirstOrDefaultAsync(u => u.RegistryName == registryName);
            if (user is null) return BadRequest("User gibt es nicht");
